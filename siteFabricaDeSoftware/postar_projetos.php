...
if($senha_log == $senha && $nivel == 0){
	...
	if($registro == true && $teste == true){
		mysqli_query($link, "INSERT INTO tb_postagensprojetos(imagem, nome, descricao, categoria, id_user) 
			VALUES('$foto', '$nome', '$descricao', '$categoria','$id_user')");

		header('location:form_postagem_projetos.php');
	...
	$sql = mysqli_query($link, "SELECT * FROM tb_postagensprojetos ORDER BY id_post DESC LIMIT 1");
	while($line = mysqli_fetch_array($sql)){
